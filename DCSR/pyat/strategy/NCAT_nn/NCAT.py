...
from pyat.model.irt_model import IRT, IRTModel
import os
...
class NCAT(nn.Module):
    def __init__(self, n_question, d_model=10, n_blocks=1, kq_same=True, dropout=0.0, policy_fc_dim=512, n_heads=1,
                 d_ff=2048, l2=1e-5, separate_qa=None, pad=0):
        super().__init__()
        """
        Input:
    ...
    def forward(self, p_0_rec, p_1_rec, p_0_target, p_1_target):
        ...
        device = self.device
        # 假设你的模型在GPU上
        self.q_embed_0 = self.q_embed_0.to(device)
        p_0_rec = p_0_rec.to(device)
        self.q_embed_1 = self.q_embed_1.to(device)
        p_1_rec = p_1_rec.to(device)
    ...
    def create_model(cls, config):
        model = cls(config.item_num, config.latent_factor, config.num_blocks,
                    True, config.dropout_rate, policy_fc_dim=512, n_heads=config.num_heads, d_ff=2048, l2=1e-5,
                    separate_qa=None, pad=0)
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        self.a_2 = self.a_2.to(self.device)
        self.b_2 = self.b_2.to(self.device)
...
class SublayerConnection(nn.Module):
    ...
    def forward(self, x, sublayer):
        ...
        self.norm = self.norm.to(torch.device('cuda'))
        self.dropout = self.dropout.to(torch.device('cuda'))
...
class MultiHeadedAttention_con(nn.Module):
    def __init__(self, h, d_model, dropout=0.1):
        ...
        self.d_k = d_model // h
        self.h = h
        self.linears = clones(nn.Linear(d_model, d_model), 5).to(torch.device('cuda'))
        self.dropout = nn.Dropout(p=dropout)
...
class env:
    def __init__(self, data, concept_map, config, T):
        ...
        # pth_path = '../../../../IRT_fisher/models/ncd/C_80_train.pt'
        current_dir = os.path.dirname(os.path.abspath(__file__))
        # 构造文件的绝对路径
        pth_path = os.path.join(current_dir, '../../../../IRT_fisher/models/ncd/DS_80_train.pt')
        name = 'NCD'
    ...
    def load_CDM(self, name, data, pth_path, config):
        if name == 'NCD':
            ...
            model.adaptest_init(data)
            model.adaptest_preload(pth_path)
    ...
    def reward(self, action, sid):
        ...
        result = self.model.adaptest_evaluate(self.tsdata)
        auc = result['auc']
        acc = result['acc']
...
class NCATModel():
    ...
    def ncat_policy(self, sid, THRESHOLD, used_actions, type, epoch):
        ...
        if type == "training":
            if len(self.memory) >= self.config['batch_size']:
                ...
                batch = [self.memory[item] for item in np.random.choice(range(len(self.memory)), (batch,))]
                data = self.convert_batch2dict(batch, epoch)
                loss = self.fa.optimize_model(data, learning_rate)
                self.tau += 5
        return
    ...
    def convert_batch2dict(self, batch, epoch):
        ...
        value[:, 0] = -500
        goals = np.max(value, axis=-1) * np.asarray(dones) * min(self.gamma,
                                                                 decay_function(max(self.config['end'] - epoch, 0) + 1,
                                                                                self.config)) + goals
